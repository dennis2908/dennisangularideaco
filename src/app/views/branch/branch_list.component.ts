...
export class BranchListComponent {
	...
	constructor(private route: ActivatedRoute, private router: Router, public formBuilder: FormBuilder, private modalService: BsModalService) {
		...
		if (Object.keys(arrAssign).length > 0) {
			if (typeof arrAssign.branchadd !== 'undefined') {
				if (arrAssign.branchadd === true) {

					this.btnadd = true;
				}
			}
			if (typeof arrAssign.branchedit !== 'undefined') {
				if (arrAssign.branchedit === true) {

					this.btnedit = true;
				}
			}
			if (typeof arrAssign.branchdelete !== 'undefined') {
				if (arrAssign.branchdelete === true) {
	...
	async onSubmit(event: any) {
		...
		if (this.userForm.valid) {
			...
			if (this.userForm.controls['id'].value) {
				await fetch("http://localhost:8000/branch/update_data/" + this.userForm.controls['id'].value, {
					method: "PUT",
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json',
				...
				await fetch("http://localhost:8000/branch/save_data", {
					method: "POST",
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json',
