...
export class RoleListComponent{
  ...
  selectEvent(item) {
   ...
   openModalAssign(data,template: TemplateRef<any>) {
	  this.formData = data;
	  
	  if(data.role_assign)
		this.dataAssign = data.role_assign
	  else
		 this.dataAssign = {}
	 
	  console.log(this.makeAssign)
	  this.modalRef = this.modalService.show(template);
   }
  ...
  onSubmit(event: any){
	 ...
	 if(this.userForm.valid){
		...
		if(this.userForm.controls['id'].value){
			fetch("http://localhost:8000/role/update_data/"+this.userForm.controls['id'].value, {
						  method: "PUT",
						  headers: {
							  'Accept': 'application/json',
							  'Content-Type': 'application/json',
		...
		else
		{
			...
			fetch("http://localhost:8000/role/save_data", {
						  method: "POST",
						  headers: {
							  'Accept': 'application/json',
							  'Content-Type': 'application/json',
