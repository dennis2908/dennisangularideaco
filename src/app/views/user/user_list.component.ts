...
export class UserListComponent {
	...
	searchData = { username: '', name: '', email: '' };
	...
	async getMRole() {
		...
		await fetch("http://localhost:8000/role/get_all_data")
			.then(res => res.json())
			.then(
				(result) => {
					this.m_role = result;
	...
	async MyfetchData(page) {
		...
		let url = "http://localhost:8000/user/get_data/" + (this.perPage * (page - 1)) + '/' + this.perPage + "/" + this.sort_id + "?" + query;
		await fetch(url)
			.then(res => res.json())
			.then(
				(result) => {
	...
	deleteItem(event) {
		this.spinnerHideShow = "display:block"
		event.preventDefault();
		fetch("http://localhost:8000/user/delete_data/" + this.idDel, {
			method: "DELETE",
			headers: {
	...
	onSubmit(event: any) {
		...
		if (this.userForm.valid) {
			...
			formData['m_role'] = this.userForm.controls['m_role'].value;
			this.spinnerHideShow = "display:block"
			this.attemptSubmit = false
			if (this.userForm.controls['id'].value) {
				...
				fetch("http://localhost:8000/user/save_data", {
					method: "POST",
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json',
